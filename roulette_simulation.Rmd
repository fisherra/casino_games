...
dry_count <- 0 
dry_count_save <- vector("double", length(short_yn))

short_yn <- head(ga_wins_yn, n = 9999)

for (i in seq_along(short_yn)) {
  if (short_yn[[i]] == FALSE) {
    dry_count = dry_count + 1
  }
  if (short_yn[[i]] == TRUE) {
    dry_count_save[[i]] = dry_count
    dry_count = 0
  } 
}
...
dry_tibble <- tibble("index" = 1:10000, "count" = dry_count_save)
